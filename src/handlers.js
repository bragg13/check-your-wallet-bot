...
const settingsKb = (settings) => {
  let kb = new Keyboard();
  kb.text(
    settings.monthlySumup
      ? "🔕 Set monthly sum-up off"
      : "🔔 Set monthly sum-up on",
  ).row();
...
const currencyKb = () => {
  ...
  kb.text("GBP").text("PLN").row();
  kb.text("DKK").text("SEK").row();
...
export async function currencyHandler(conversation, ctx) {
  await ctx.reply(
    "Okay, choose a currency from below or type in a custom one:",
    {
